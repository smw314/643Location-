...
fb_2022 <- ml_data %>% 
  filter(tagged_pitch_type %in% fb,
...
racing_results <- tune_race_anova(
  xgb_spec,
  recipe_spec,
  resamples = bootstraps(fb_2022, times = 5),
  grid = grid_vals
)
...
predictions <- fit_final_model %>%
  predict(fb_2023_test %>% select(-pitch_rv)) %>%
  bind_cols(fb_2023)

xgb_booster <- fit_final_model$fit$fit$fit

xgb.save(xgb_booster, "xgb_fb_model.model")

write_csv(predictions, 'fb_preds.csv')
