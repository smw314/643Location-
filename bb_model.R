...
bb_2022 <- ml_data %>% 
  filter(tagged_pitch_type %in% bb,
...
racing_results <- tune_race_anova(
  xgb_spec,
  recipe_spec,
  resamples = bootstraps(bb_2022, times = 5),
  grid = grid_vals
)
...
predictions <- fit_final_model %>%
  predict(bb_2023_test %>% select(-pitch_rv)) %>%
  bind_cols(bb_2023)

xgb_booster <- fit_final_model$fit$fit$fit

xgb.save(xgb_booster, "xgb_bb_model.model")

write_csv(predictions, 'bb_preds.csv')
...
X_train_matrix <- as.matrix(bb_2022 %>% select(-pitch_rv))
